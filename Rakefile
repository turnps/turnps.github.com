...
task :post do
  ...
  open(filename, 'w') do |post|
    ...
    post.puts 'summary: '
    post.puts 'source: false'
    post.puts 'author: '
    post.puts "category: "
    post.puts "tags: #{tags}"
    post.puts "published: true"
    post.puts "comments: true"
    #post.puts "legacydate: 3/15/2010 05:00"
    #post.puts "legacyId: 530"
    post.puts ""
    post.puts "---"
    #post.puts "{% assign align='right'%}"
    #post.puts "{% assign src='/jruby/logo.png'%}"
    #post.puts "{% include image.html %}"
  end
end # task :post

# Usage: rake redirect title="A Title" [date="2012-02-09"] [tags=[tag1, tag2]]
desc "Begin a new post in #{CONFIG['posts']}"
task :redirect do
  abort("rake aborted: '#{CONFIG['posts']}' directory not found.") unless FileTest.directory?(CONFIG['posts'])
  title = ENV["title"] || "new-post"
  ...
  puts "Creating new redirect: #{filename}"
  open(filename, 'w') do |post|
    post.puts "---"
    post.puts "layout: redirect"
    post.puts "title: \"#{title.gsub(/-/,' ')}\""
    post.puts 'summary: '
